...
def build_field_by_id_resolver(self):
    def field_resolver_function(parent, info, **kwargs):
        ...
        if self._session_manager != None:
            valid, actual_user_instance, error = self._session_manager.validate_access(
                info.context, access_group
            )
        else:
            valid = True
        if valid:
            ...
            if "field_by_id" in config["validators_by_operation"]:
                valid_operation = evaluate_result(
                    config["validators_by_operation"]["field_by_id"],
                    info,
                    result,
                    **kwargs,
                )
    ...
    def list_resolver_function(parent, info, **kwargs):
        ...
        for filter_config in external_filters:
            ...
            if param_value is not None:
                if query_object is None:
                    ...
                    query_object.add(
                        Q(**{filter_config.get("field_name"): param_value}),
                        filters_operator,
                    )
        for filter_config in internal_filters:
            ...
            if value_filter == None:
                if on_return_none == "skip":
                    ...
                    if query_object is None:
                        query_object = Q(
                            **{f"{filter_config.get('field_name')}__isnull": True}
                        )
                    else:
                        query_object.add(
                            Q(**{f"{filter_config.get('field_name')}__isnull": True}),
                            filters_operator,
                        )
            else:
                if query_object is None:
                    ...
                    query_object.add(
                        Q(**{filter_config.get("field_name"): value_filter}),
                        filters_operator,
                    )
        if query_object is None:
            query_object = Q()
        ...
        if valid:
            ...
            if pagination_length == 0:
                ...
                if type(ordering_field) in [list, tuple]:
                    items = model.objects.filter(query_object).order_by(
                        *ordering_field
                    )[inicio:fin]
                else:
                    items = model.objects.filter(query_object).order_by(ordering_field)[
                        inicio:fin
                    ]
                callbacks = config.get("callbacks_by_operation").get("list_field")
                if callbacks is not None:
...
def get_return_object(model_config):
    ...
    if (
        model_config.get("pagination_length") == 0
        or model_config.get("pagination_style") == "infinite"
    ):
        return_object = graphene.List(model_config["type"], filters_args)
    elif (
        model_config.get("pagination_length") > 0
        and model_config.get("pagination_style") == "paginated"
    ):
        return_object = graphene.Field(model_config["paginated_type"], filters_args)
    else:
        raise Exception(
            f"Unknown pagination style {model_config.get('pagination_style')}"
        )
