    ...
    for field in model._meta.fields:
        if field.name not in exclude_fields:
            ...
            if field_type in MODEL_FIELD_TO_GRAPHENE_TYPE.keys():
                ...
                if (
                    field.null
                    or field_type in ["ImageField", "FileField"]
                    or field.name in fields_as_password
                ):
                    required = False
                setattr(Arguments, field.name.lower(), argument_type(required=required))
    for arg in custom_args:
...
def build_mutate_for_create(self):
    ...
    def mutate_create_function(parent, info, **kwargs):
        ...
        if (
            operation_name == "__typename"
            and len(info.operation.selection_set.selections) > 1
        ):
            operation_name = info.operation.selection_set.selections[
                1
            ].name.value.lower()
        if operation_name not in self._models_by_op_name.keys():
            raise Exception(
                f"No se encontró la operación {operation_name} en el esquema"
            )
        ...
        if self._session_manager != None:
            valid, actual_user_instance, session_error = (
                self._session_manager.validate_access(info.context, access_group)
            )
        else:
            valid = True
        ...
        return_object = type(
            ...
            (graphene.ObjectType,),
            {
                ...
                model.__name__.lower(): graphene.Field(config["type"]),
                ...
                for key, value in kwargs.items():
                    ...
                    if value != None:
                        if field_type == "ForeignKey" or field_type == "OneToOneField":
                            ...
                            if (
                                model._meta.get_field(key).choices != None
                                and len(model._meta.get_field(key).choices) > 0
                            ):
                                choices = model._meta.get_field(key).choices
                                valid_options = [c[0] for c in choices]
                                if value not in valid_options:
                                    raise Exception(
                                        f"{value} no es una opción válida para {key}"
                                    )
                            setattr(instance, key, value)
                # evaluate validators
                valid_operation = True
                if "create_field" in config["validators_by_operation"]:
                    valid_operation = evaluate_result(
                        config["validators_by_operation"]["create_field"],
                        info,
                        instance,
                        **kwargs,
                    )
                if valid_operation:
                    ...
                    return return_object(
                        **{
                            ...
                            "error": ErrorManager.get_error_by_code(NO_ERROR),
                        }
                    )
                else:
                    return return_object(
                        **{
                            "estado": False,
                            "error": ErrorManager.get_error_by_code(
                ...
                return return_object(
                    **{
                        ...
                        "error": ErrorManager.get_error_by_code(
                            ...
                            custom_message="Error Inesperado",
            ...
            try:
                if model.objects.filter(id=kwargs.get("id")).exists():
                    ...
                    if "update_field" in config["validators_by_operation"]:
                        valid_operation = evaluate_result(
                            config["validators_by_operation"]["update_field"],
                            info,
                            instance,
                            **kwargs,
                        )
                    if valid_operation:
                        # get internal resolvers
                        internal_field_resolvers = config.get(
                            "internal_field_resolvers"
                        )
                        if "update_field" in internal_field_resolvers.keys():
                            fields_to_resolve = internal_field_resolvers.get(
                                "update_field"
                            )
                            kwargs.update(fields_to_resolve)
                        for key, value in kwargs.items():
                            if key != "id":
                                ...
                                if value != None:
                                    if field_type == "ForeignKey":
                                        foreign_model = instance._meta.get_field(
                                            key
                                        ).related_model
                                        value = foreign_model.objects.get(id=value)
                                        setattr(instance, key, value)
                                        ...
                                        if (
                                            model._meta.get_field(key).choices != None
                                            and len(model._meta.get_field(key).choices)
                                            > 0
                                        ):
                                            choices = model._meta.get_field(key).choices
                                            valid_options = [c[0] for c in choices]
                                        ...
                                        setattr(instance, key, value)
                        instance.save()
                        callbacks = config.get("callbacks_by_operation").get(
                            "update_field"
                        )
                        if callbacks is not None:
                            for callback in callbacks:
                    ...
                    return return_object(
                        **{
                            "estado": False,
                            "error": ErrorManager.get_error_by_code(INSTANCE_NOT_FOUND),
                        }
                    )
            except Exception as e:
                return return_object(
...
def build_mutate_for_delete(self):
    def mutate_delete_function(parent, info, **kwargs):
        ...
        return_object = type(
            info.return_type.name,
            (graphene.ObjectType,),
            {"estado": graphene.Boolean(), "error": graphene.Field(ErrorMsgType)},
        )
        if valid:
            try:
                if model.objects.filter(id=kwargs.get("id")).exists():
                    ...
                    if "delete_field" in config["validators_by_operation"]:
                        valid_operation = evaluate_result(
                            config["validators_by_operation"]["delete_field"],
                            info,
                            instance,
                            **kwargs,
                        )
...
def get_fields_to_ignore(model_config, operation):
    ...
    if operation in model_config.get("exclude_fields_by_operation").keys():
        fields_to_ignore += model_config.get("exclude_fields_by_operation").get(
            operation
        )
